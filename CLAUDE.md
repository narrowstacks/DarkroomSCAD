...
- `Heat_Set_Screw_Size` ("M2" | "M2.5" | "M3"), `Heat_Set_Screw_Head` (socket | button | pan | cheese | custom) + `Heat_Set_Screw_Head_Diameter`, `Heat_Set_Thread_Hole_Adjust`, `Heat_Set_Head_Hole_Adjust`: heat-set peg screw sizing (see `HEAT_SET_SCREW_SIZES` / `HEAT_SET_SCREW_HEADS`); carrier.scad resolves them to two hole diameters that the peg chain carries
...
- `HEAT_SET_SCREW_SIZES`: per-size `[name, thread hole dia]` (M2 / M2.5 / M3); `HEAT_SET_SCREW_HEADS`: per head style `[style, [dk for M2, M2.5, M3]]` (ISO max head diameters). `heat_set_thread_hole_dia()` adds `HEAT_SET_HOLE_FDM_COMPENSATION = 0.3` (M2 → 1.9mm modelled, prints ≈ tap size on FDM); `heat_set_head_hole_dia()` adds `2 × PEG_HOLE_TOLERANCE` to the head diameter (or a custom measured one)

...
module tube(length, outer_diameter, inner_diameter, thickness) {
    ...
    union() {
        ...
        for (angle = [0, 180]) {
            rotate([0, 0, angle])
                translate([inner_diameter / 2 - ROTARY_STICK_HOLDER_LENGTH, -ROTARY_STICK_HOLDER_WIDTH / 2, total_base_height + length - 22 - ROTARY_STICK_HOLDER_HEIGHT])
